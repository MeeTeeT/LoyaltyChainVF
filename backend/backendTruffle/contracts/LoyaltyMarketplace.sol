...
contract LoyaltyMarketplace is ERC721URIStorage {
  ...
  //the event emitted when a token is successfully listed
    event UserAccountCreated (
        ...
        string image,
        bool isABrand, 
        bool isBrandRegisterOnPlatform,
        bool isAddressAlreadyCreatedAccount
    );
    ...
    function createTokenToMarketplace(string memory tokenURI, uint256 price) public payable returns (uint) {
    ...
    //The first time a token is created, it is listed here
    function createTokenToAddress(string memory tokenURI, address _addr) public payable returns (uint) {
        //Increment the tokenId counter, which is keeping track of the number of minted NFTs
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
    ...
    function getBrandFromId(uint _idBrand) public view returns (UserAccount memory){
        return idToListedBrand[_idBrand];
    }
